# Status recon — the six resorts Indy added for 2026-27

**Date: 2026-09-02.** Written while adding the 26/27 partners to the IndiePeaks
...
- **67 resorts are published but unread by the app.** The feed carries 250
  resorts; `status-sources.json` reads 185. The gap is mostly European and
  Japanese (`abondance`, `avoriaz`, `levi`, `kiroro`, `geto-kogen`,
  `muica-snow-resort`, …). Not a bug in either repo, just an app-side wiring
  pass that has not been run since 2026-08-05 — `Scripts/sync-status-sources.py`
  in the app repo generates them from the live payloads. Out of scope for the
  26/27 roster change; worth its own pass before the season starts.
